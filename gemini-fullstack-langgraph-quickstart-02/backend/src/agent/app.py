...
from .graph import finalize_answer, graph  # Add this import at the top
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        state = {
            ...
            "initial_search_query_count": 2,  # Default, or make configurable
            "max_research_loops": 1,          # Default, or make configurable
            "reasoning_model": "gemini-2.5-flash-lite-preview-06-17",  # Default, or make configurable
        }

        try:
            # Use the graph to process the message (finalize_answer is the last node)
            response = await graph.ainvoke(state)
            print("Graph response:", response)  # Debug print

            # Extract the answer from the response
            if isinstance(response, dict) and "messages" in response and response["messages"]:
                messages = response["messages"]
                ai_message = messages[-1] if len(messages) > 1 else messages[0]
                ...
                answer = "I apologize, but I couldn't process your request properly."
        except Exception as e:
